      ...
      box.addEventListener("click", () => {
        if (box.textContent === "") {
          ...
          if (winner) {
            ...
            playersData[playersData.length - 1].scorePlayerOne =
              scores[playerX];
            playersData[playersData.length - 1].scorePlayerTwo =
              scores[playerO];
  ...
  function checkWin() {
    ...
    for (let row = 0; row < gridSize; row++) {
      for (let col = 0; col < gridSize; col++) {
        if (grid[row][col] !== "") {
          if (
            checkDirection(grid, row, col, 1, 0, winCondition) ||
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function displayHistory() {
    ...
    if (playersData.length === 0) {
      historyList.innerHTML =
        '<tr><td colspan="5">No game history available.</td></tr>';
    } else {
